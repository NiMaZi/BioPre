...
def upload_to_S3(_inpath,_fname,_counter,_format):
    ...
    data=f.read()
    f.close()
    targetBucket.put_object(Body=data,Key="yalun/EEG_expansion/"+_fname+str(_counter)+"."+_format)

logf=open(homedir+"/results/logs/annotator_eeg_expansion.txt",'a')
for i in range (start,end):
    try:
        sourceBucket.download_file("yalun/EEG_expansion/abs"+str(i)+".txt",homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt")
        txt_path=homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt"
        path=get_annotation(txt_path)
        upload_to_S3(path,"abs",i,"csv")

        sourceBucket.download_file("yalun/EEG_expansion/body"+str(i)+".txt",homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt")
        txt_path=homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt"
        path=get_annotation(txt_path)
        upload_to_S3(path,"body",i,"csv")

        sourceBucket.download_file("yalun/EEG_expansion/title"+str(i)+".txt",homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt")
        txt_path=homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt"
        path=get_annotation(txt_path)
