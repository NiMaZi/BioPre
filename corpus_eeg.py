...
def upload_to_S3(_inpath,_fname,_counter,_format):
    ...
    data=f.read()
    f.close()
    targetBucket.put_object(Body=data,Key="yalun/port/spectroscopy/"+_fname+str(_counter)+"."+_format)

logf=open(homedir+"/results/logs/annotator_log_port.txt",'a')
for i in range (start,end):
    try:
        sourceBucket.download_file("yalun/port/spectroscopy/abs"+str(i)+".txt",homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt")
        txt_path=homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt"
        path=get_annotation(txt_path)
        upload_to_S3(path,"abs",i,"csv")

        sourceBucket.download_file("yalun/port/spectroscopy/body"+str(i)+".txt",homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt")
        txt_path=homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt"
        path=get_annotation(txt_path)
        upload_to_S3(path,"body",i,"csv")

        sourceBucket.download_file("yalun/port/spectroscopy/title"+str(i)+".txt",homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt")
        txt_path=homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt"
        path=get_annotation(txt_path)
