...
for entry in featured_list:
	...
	for a_key in abs_dict.keys():
		for b_key in body_dict.keys():
			sample_prelist.append([a_key,b_key,abs_dict[a_key][0],abs_dict[a_key][1]-abs_dict[a_key][0],abs_dict[a_key][2],body_dict[b_key][0],body_dict[b_key][1]-body_dict[b_key][0],body_dict[b_key][2]])
