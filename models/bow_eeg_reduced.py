...
def train_on_batch_S3(_model,_source,_volume,_bcount,_batch,_mbatch,_epochs=5):
	...
	for i in range(0,_volume):
		...
		abs_count=0.0
		try:
			bucket.download_file("yalun/"+_source[0]+"/abs"+str(7*i)+".csv",homedir+"/temp/tmp0.csv")
		except:
			continue
		...
		body_vec=[0.0]
		try:
			bucket.download_file("yalun/"+_source[0]+"/body"+str(7*i)+".csv",homedir+"/temp/tmp0.csv")
		except:
			continue
		with open(homedir+"/temp/tmp0.csv",'r',encoding='utf-8') as cf:
			rd=csv.reader(cf)
		...
		if len(sample_list)>=_batch:
			...
			updata=s3f.read()
			bucket.put_object(Body=updata,Key="yalun/results/models/MLPsparse_1hidden_eegrdc2.h5")
			s3f.close()
