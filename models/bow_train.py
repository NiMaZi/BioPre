...
def train_on_batch_S3(_model,_source,_volume,_bcount,_batch,_mbatch,_epochs=5):
	...
	for i in range(0,_volume):
		...
		if len(sample_list)>=_batch:
			...
			Y_strain=np.clip(Y_train-np.ceil(X_train),0.0,1.0)
			_model.fit(X_train,Y_strain,batch_size=_mbatch,verbose=1,epochs=_epochs,validation_split=1.0/16.0,callbacks=[early_stopping,early_stopping_val])
			try:
