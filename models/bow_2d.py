...
def train_on_batch_S3(_model,_volume,_batch,_mbatch,_epochs=1):
	...
	for i in range(0,_volume):
		...
		abs_count=0.0
		bucket.download_file("yalun/annotated_papers_with_txt/abs"+str(i)+".csv",homedir+"/temp2/tmp.csv")
		with open(homedir+"/temp2/tmp.csv",'r',encoding='utf-8') as cf:
			rd=csv.reader(cf)
			for item in rd:
				if item[0]=="Mention":
					continue
		...
		body_count=0.0
		bucket.download_file("yalun/annotated_papers_with_txt/body"+str(i)+".csv",homedir+"/temp2/tmp.csv")
		with open(homedir+"/temp2/tmp.csv",'r',encoding='utf-8') as cf:
			rd=csv.reader(cf)
			for item in rd:
				if item[0]=="Mention":
		...
		if len(sample_list)>=_batch:
			...
			try:
				os.remove(homedir+"/results/models/temp_model_2hidden.h5")
			except:
				pass
			_model.save(homedir+"/results/models/temp_model_2hidden.h5")
			s3f=open(homedir+"/results/models/temp_model_2hidden.h5",'rb')
			updata=s3f.read()
			bucket.put_object(Body=_model,Key="yalun/results/models/MLPsparse_2hidden_"+str(batch_count)+".h5")
			s3f.close()
			batch_count+=1
			sample_list=[]
	if len(sample_list):
