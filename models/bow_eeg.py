...
def train_on_batch_S3(_model,_source,_volume,_bcount,_batch,_mbatch,_epochs=5):
	...
	for i in range(0,_volume):
		...
		if len(sample_list)>=_batch:
			...
			updata=s3f.read()
			bucket.put_object(Body=updata,Key="yalun/results/models/MLPsparse_1hidden_eeggputest.h5")
			s3f.close()
			logf=open(homedir+"/results/logs/bow_training_log_eeg.txt",'a')
			logf.write("eeg_gpu_test,%s,%d\n"%(str(_source),batch_count))
			logf.close()
			batch_count+=1
			sample_list=[]
	if len(sample_list):
...
if __name__=="__main__":
	...
	model,bcount=train_on_batch_S3(model,source_key,1000,0,17,16)