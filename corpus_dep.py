...
def upload_to_S3(_inpath,_fname,_counter,_format):
    ...
    data=f.read()
    f.close()
    targetBucket.put_object(Body=data,Key="yalun/Dependence/"+_fname+str(_counter)+"."+_format)
...
for i in range (start,end):
    try:
        ...
        sourceBucket.download_file("yalun/Dependence/title"+str(i)+".txt",homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt")
        txt_path=homedir+"/thesiswork/source/papers/tmp"+str(pid)+".txt"
        path=get_annotation(txt_path)
